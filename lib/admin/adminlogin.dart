...
class _LoginPageAdminState extends State<LoginPageAdmin> {
  ...
  Future<void> _login() async {
    try {
      ...
      var userQuery = await _firestore.collection('admin').where('email', isEqualTo: email).get();
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => JobDashboard(uid: uid,)),
      );
      ScaffoldMessenger.of(context).showSnackBar(
            ...
            children: [
              ...
              TextButton(
                onPressed: () {
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(builder: (context) => SignUpPageAdmin()),
                  );
                },
                child: Text(
                  'Create an account',
